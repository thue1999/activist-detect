...
columns_to_drop = ["yq", "index", "t_wc", "liwc_WC", "year1", "companyid", "F1_num_diligent_HF", "F1_num_factset_diligent_HF", "F1_num_shareholder_proposal", "comname", "isin", "ticker", "cusip", "cusip6", "cusip8", "cik", "state", "conm", "exchg", "loc", "sic_2_digit", "sic_3_digit", "ff17", "ff18", "ff48", "F1_partial_HF_own", "F1_primary_HF_own", "F1_primary_partial_HF_own", "F1_HF", "F1_factset_HFactivism", "F1_num_factset_activism", "F1_num_factset_HFactivism", "F1_factset_activism", "F1_diligent_HF", "F1_HF_own"]
...
class DataProcessor:
    ...
    def load_data(self):
        ...
        df.to_csv(output_dir / "raw_data929.csv", index=False)
...
def save_rfecv_results(selected_features, industry, neighbors):
    try:
        ...
        results_table_filename = output_dir / f"all_rfecv_results_sic_{int(industry)}_neighbors_{neighbors}929.csv"
...
def compute_and_save_shap_values(estimator, features, labels, selected_features, industry, neighbors):
    try:
        ...
        shap_results_filename = output_dir / f"all_shap_values_sic_{int(industry)}_neighbors_{neighbors}929.csv"
...
def main():
    ...
    results_df.to_excel(output_dir / "rfecv_summary_results929.xlsx", index=False)
    ...
    summary_df.to_excel(output_dir / "all_selected_features_summary929.xlsx", index=False)
    ...
    neighbors_df.to_excel(output_dir / "best_neighbors_summary929.xlsx", index_label='Industry')
    hit_rates.to_csv(output_dir / 'hit_rate_summary929.csv', index=False)
