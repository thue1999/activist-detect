...
selected_features_summary = pd.read_excel(output_dir / "all_selected_features_summary929.xlsx")
...
def train_and_save_model_all(train_features, train_labels, pipeline, neighbors, selected_features):
    ...
    # Save the model
    model_filename = output_dir / f"model2_sic_ALL_neighbors_{neighbors}.pkl"
    joblib.dump(pipeline, model_filename)
    logging.info(f"Model saved for ALL with {neighbors} neighbors")

    # Save the feature names
    feature_names_filename = output_dir / f"feature_names2_sic_ALL_neighbors_{neighbors}.pkl"
    with open(feature_names_filename, 'wb') as f:
        pickle.dump(train_features.columns.tolist(), f)
...
def main():
    ...
    pd.Series(df.columns).to_csv('columns.csv', index=False)
    ...
    test_data.to_csv('test_data2022.csv')
    ...
    results_df.to_excel(output_dir / "hyperparameter_tuning_results.xlsx", index=False)
    logging.info(f"Saved hyperparameter tuning results to {output_dir / 'hyperparameter_tuning_results.xlsx'}")
    '''
    ...
    target_scores = target_df["F1_factset_diligent_HF"]
    ...
    for _, row in test_sample.iterrows():
        ...
        model_filename_all = output_dir / f"model2_sic_ALL_neighbors_{NEIGHBORS_ALL}.pkl"
        ...
        industry_model_filename = output_dir / f"model2_sic_{int(industry)}_neighbors_{neighbors}.pkl"
        industry_feature_names_filename = output_dir / f"feature_names2_sic_{int(industry)}_neighbors_{neighbors}.pkl"
        ...
        with open(output_dir / f"model2_sic_ALL_neighbors_{NEIGHBORS_ALL}.pkl", 'rb') as model_file_all:
            model_all = pickle.load(model_file_all)
        with open(output_dir / f"feature_names2_sic_ALL_neighbors_{NEIGHBORS_ALL}.pkl", 'rb') as f_all:
    ...
    print(industries)
    confidence_scores_df.insert(2, "F1_factset_diligent_HF", target_scores)
    ...
    with pd.ExcelWriter(output_dir / "confidence_scoresnew4baseline2.xlsx") as writer:
